...
function rnd(range = [0, 1], precision = 12) {
  const [min, max] = range;
...
const namedColors = new ColorMap();
...
function random(model = 'rgb', descriptor = {}) {
  if (typeof model !== 'string') return undefined;
  model = model.trim().toLowerCase();
...
function randomContrast(base = 'white', descriptor = {
  hue: DEG_RANGE,
  saturation: ONE_RANGE,
  chroma: CHROMA_RANGE,
  targetContrast: [0, 21],
...
export { DisplayP3Color, LabColor, XYZColor, color, contrast, lerp, mix, random, randomContrast, sRGBColor };
